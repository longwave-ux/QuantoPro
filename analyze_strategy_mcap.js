        ...
        try {
            ...
            for (let i = ltfData.length - backtestCandles; i < ltfData.length - 100; i++) {
                ...
                if (analysis.score >= activeConfig.THRESHOLDS.MIN_SCORE_SIGNAL && analysis.setup) {
                    ...
                    // MCAP ANALYSIS
                    // Requires mcap_cache.json in CWD
                    if (!global.mcapCache) {
                        try {
                            const fs = await import('fs/promises');
                            const data = await fs.readFile('./mcap_cache.json', 'utf-8');
                            global.mcapCache = JSON.parse(data);
                        } catch { global.mcapCache = {}; }
                    }

                    // Extract Base Symbol (e.g., BTCUSDT -> BTC)
                    const baseSym = symbol.replace('USDT', '').replace('USDC', '').replace('PERP', '');
                    const mcap = global.mcapCache[baseSym] || 0;

                    if (!stats.byMcap) stats.byMcap = {
                        "Mega_GT_10B": { wins: 0, losses: 0, total: 0 },
                        "Large_1B_10B": { wins: 0, losses: 0, total: 0 },
                        "Mid_100M_1B": { wins: 0, losses: 0, total: 0 },
                        "Small_LT_100M": { wins: 0, losses: 0, total: 0 },
                        "Unknown": { wins: 0, losses: 0, total: 0 }
                    };

                    let tier = "Unknown";
                    if (mcap > 10000000000) tier = "Mega_GT_10B";
                    else if (mcap > 1000000000) tier = "Large_1B_10B";
                    else if (mcap > 100000000) tier = "Mid_100M_1B";
                    else if (mcap > 0) tier = "Small_LT_100M"; // 0 means unknown

                    stats.byMcap[tier].total++;
                    if (outcome.result === 'WIN') stats.byMcap[tier].wins++;
                    if (outcome.result === 'LOSS') stats.byMcap[tier].losses++;
